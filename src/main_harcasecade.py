        ...
        while self.running:
            ...
            if face_found:
                for (x, y, w, h) in faces:
                    ...
                    # ======== LOGIKA KEDIP / TERTUTUP ========
                    if len(eye_dark_ratios) > 0:
                        avg_eye_dark = sum(eye_dark_ratios) / len(eye_dark_ratios)

                        if avg_eye_dark < 0.03:   # threshold mata tertutup
                            closed_counter += 1
                            eyes_closed = True
                        else:
                            closed_counter = 0
                ...
                if sleepy and not self.was_sleepy:
                    ...
                    if self.was_sleepy:
                        if self.popup is not None and self.popup_close_time is None:
                            self.popup_close_time = time.time() + 5
                ...
                self.status_label.config(
                    text="⚠ MENGANTUK!" if sleepy else "Status: NORMAL",
                    bg="red" if sleepy else "green"
                )
